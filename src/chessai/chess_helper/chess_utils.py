...
def vec_to_fen(feature_vector: np.array):
    """
    ...
    # Exclude side to move, castling rights, en passant, halfmove clock, and fullmove number
    board_state = feature_vector[:-71]
    ...
    for piece_idx in range(12):
        for color_idx in range(2):
            piece_type = CHESS_PIECES[piece_idx]
            color = CHESS_COLOURS[color_idx]
    ...
    fen = ' '.join([fen_board, fen_side_to_move, fen_castling_rights or '-',
...
def action_index_to_move(action_index: int) -> chess.Move:
    ...
    Args:
      ...
      - 9 pawn promotions: 3 promotions (queen promotion is default) for 3 possible captures
    ...
    if plane_of_execution < 8*7:
        ...
        rank_diff, file_diff = knight_moves[knight_move_index]
...
def move_to_action_index(move: chess.Move) -> int:
    """
    Convert the given UCI move string to an action index.
    """
    tot_planes_of_execution = (8*7 + 8 + 9)
    starting_square = move.from_square
    starting_square_index = chess.SQUARE_NAMES.index(chess.SQUARE_NAMES[starting_square])
    target_square = move.to_square
    target_square_index = chess.SQUARE_NAMES.index(chess.SQUARE_NAMES[target_square])
    promotion = move.promotion

    rank_diff = (target_square_index // 8) - (starting_square_index // 8)
    file_diff = (target_square_index % 8) - (starting_square_index % 8)

    if promotion and promotion != chess.QUEEN:
        # Queen promotion is default -> 0 value for it
        promotions = [chess.KNIGHT, chess.BISHOP, chess.ROOK]
        promotion_index = promotions.index(promotion)
        pawn_move = 0
        if rank_diff == 1 and file_diff == 0:
            pawn_move = 0
        elif rank_diff == 1 and file_diff == 1:
            pawn_move = 1
        elif rank_diff == 1 and file_diff == -1:
            pawn_move = 2
        return starting_square_index * tot_planes_of_execution + 8*7 + 8 + pawn_move*3 + promotion_index
    ...
    if (rank_diff, file_diff) in knight_moves:
        return starting_square_index * tot_planes_of_execution + 8*7 + knight_moves.index((rank_diff, file_diff))

    # Add other move types (e.g., rook, bishop, queen, king, pawn non-promotion) here
    directions = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW']
    if file_diff == 0:
        if rank_diff > 0:
            direction = 'N'
        ...
        if rank_diff < 0:
            direction = 'SE'
    return starting_square_index * tot_planes_of_execution + directions.index(direction)
...
fen = '8/k6K/6P1/8/8/8/1p6/P7 w - - 0 1'
...
move = chess.Move(chess.parse_square('g6'), chess.parse_square('g7'))

action_index = move_to_action_index(move)
print(action_index_to_move(action_index))

board.push(move)

move = chess.Move(chess.parse_square('b2'), chess.parse_square('a1'), chess.KNIGHT)
